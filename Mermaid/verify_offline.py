"""
mermaid 离线编辑器断网验证：
- 拦截所有 http(s) 并 abort（模拟纯离线）
- 打开 dist/index.html (file://)
- 等默认图渲染（#preview 内出现 svg）
- 真实输入新 mermaid 代码 → 预览更新
- 点“导出 SVG”
- 统计外部请求 / 错误
"""
import json
import sys
from pathlib import Path
from playwright.sync_api import sync_playwright

DIST = Path(__file__).parent / "dist" / "index.html"
URL = DIST.as_uri()

external_requests = []
page_errors = []
console_errors = []


def main():
    if not DIST.exists():
        print(json.dumps({"ok": False, "reason": f"dist not found: {DIST}"}, ensure_ascii=False))
        sys.exit(1)

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        ctx = browser.new_context(viewport={"width": 1280, "height": 800})
        page = ctx.new_page()

        def on_route(route):
            u = route.request.url
            if u.startswith("http://") or u.startswith("https://"):
                external_requests.append(u)
                return route.abort()
            return route.continue_()

        page.route("**/*", on_route)
        page.on("pageerror", lambda e: page_errors.append(str(e)))
        page.on("console", lambda m: console_errors.append(m.text) if m.type == "error" else None)

        page.goto(URL, wait_until="domcontentloaded", timeout=60000)
        # 等默认图渲染
        page.wait_for_selector("#preview svg", timeout=30000)
        page.wait_for_timeout(800)
        svg_before = page.evaluate("document.querySelectorAll('#preview svg').length")
        text_before = page.evaluate(
            "(() => { const t = document.querySelector('#preview svg'); return t ? t.textContent : ''; })()"
        )

        # 真实输入新代码（清空后填入一个不同图）
        new_code = "sequenceDiagram\n  Alice->>Bob: 你好\n  Bob-->>Alice: 收到"
        ta = page.locator("textarea")
        ta.fill(new_code)
        page.wait_for_timeout(1200)
        svg_after = page.evaluate("document.querySelectorAll('#preview svg').length")
        text_after = page.evaluate(
            "(() => { const t = document.querySelector('#preview svg'); return t ? t.textContent : ''; })()"
        )

        # 导出 SVG
        try:
            page.get_by_text("导出 SVG", exact=True).click(timeout=8000)
            page.wait_for_timeout(500)
            export_ok = True
        except Exception as e:
            export_ok = False
            console_errors.append("export: " + str(e))

        page.screenshot(path=str(Path(__file__).parent / "verify_screenshot.png"))
        browser.close()

    result = {
        "ok": True,
        "file": str(DIST),
        "svg_before": svg_before,
        "svg_after": svg_after,
        "text_before_has_开始": "开始" in (text_before or ""),
        "text_after_has_Alice": "Alice" in (text_after or ""),
        "export_clicked": export_ok,
        "external_requests": external_requests,
        "external_request_count": len(external_requests),
        "page_errors": page_errors,
        "page_error_count": len(page_errors),
        "console_errors": console_errors,
        "console_error_count": len(console_errors),
    }
    print(json.dumps(result, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
